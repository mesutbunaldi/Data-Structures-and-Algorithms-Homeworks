...
public class BTree<E extends Comparable<E>>  implements SearchTree<E> {
    ...
    /**
     *
     * @param target searching target element
     * @param items  my array
     * @param first  first element position
     * @param last   last element position
    ...
    private int binarySearch(E target, E[] items, int first, int last) {
        if (first >= last)
            ...
            int middle = first+(last-first) / 2; // Next probe index.
     ...
     */
    private boolean insert(Node<E> root, E item) {//private int binarySearch(E item, E[] data, int start, int last)
